...
//,ItemListener
{
   ...
   JComboBox bname,nbname,stat,tf1;
     ...
     public bill1()
      {
            ...
            try
            {
            	tf1=new JComboBox();
                    ...
                    s2=con.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
                     s3=con.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
                   rs2 = s2.executeQuery("select * from custinfo");
                    	tf1.removeAllItems();
                    	while(rs2.next())
                        {
                         tf1.addItem(rs2.getString(2));
                         }
            ...
            bg1=new ButtonGroup();
                ...
                r1=new JRadioButton("Indoors");
                r1.setBounds(250,160,100,20);
		        r2=new JRadioButton("Outdoors");
		        r2.setBounds(360,160,100,20);
		        r3=new JRadioButton("Accessories");
		...
		l1=new JLabel("! Sai Sports Agency !");
		l2=new JLabel("Ph.No :-9881151060");
        ...
        class  KeyListen extends KeyAdapter
        {
               ...
               {
               try{
                     rs1 = s1.executeQuery("select * from Indoors");
                     while(rs1.next())
                     {
                     if(bname.getSelectedItem().equals(rs1.getString(2)))
                     {
                          if(Integer.parseInt(txtQB.getText())> Integer.parseInt(rs1.getString(6)))
                            {
               ...
               if(obj==txtQN){
               try{
                     rs1 = s1.executeQuery("select * from Outdoors");
                     while(rs1.next())
                     {
                     if(nbname.getSelectedItem().equals(rs1.getString(2))){
                          if(Integer.parseInt(txtQN.getText())> Integer.parseInt(rs1.getString(6)))
                            {
                             String msg= "Quantity is Greater than Available Stock ";
                             txtQN.setText("");
                             txtPN.setText("");
               ...
               if(obj==txtQS){
               try{
                     rs1 = s1.executeQuery("select * from Accessories");
                     while(rs1.next()){
        ...
        public void actionPerformed(ActionEvent e1)
       {
             ...
             if(obj==tf1){
                         rs3=null;
                         try{
                         	 rs3 = s3.executeQuery("select * from custinfo");
                     while(rs3.next()){
                     if(tf1.getSelectedItem().equals(rs3.getString("custname")))
                     {
               ...
               if(obj==btnAdd)
               {
                     if(bname.isEnabled())
                     {
                           ...
                           {
                               j=Integer.parseInt(txtQB.getText());
                           }
                           i=i-j;
                           try{
                               s1=null;
                                 ...
                                 String q = "UPDATE Indoordetails SET quantity='"+s+"'"+
                                  "WHERE Indoor_name='"+bname.getSelectedItem()+"'";
                                  s1.executeUpdate(q);
                           }
                           catch(Exception ex){
                               System.out.println(ex);
                    ...
                    if(nbname.isEnabled())
                    {
                          ...
                          int j=0;
                           ...
                           {
                               j=Integer.parseInt(txtQN.getText());
                           }
                           i=i-j;
                           try{
                                 s1=null;
                                 ...
                                 s1 = con.createStatement();
                                 String s = String.valueOf(i);
                                 String q = "UPDATE OutdoorDetails SET quantity='"+s+"'"+
                     ...
                     else if(stat.isEnabled())
                     {
                          ...
                          int j=0;
                           ...
                           {
                               j=Integer.parseInt(txtQS.getText());
                           }
                           i=i-j;
                           try{
                                 s1=null;
                                 ...
                                 s1 = con.createStatement();
                                 String s = String.valueOf(i);
                                 String q = "UPDATE Accessories SET Quantity='"+s+"'"+
